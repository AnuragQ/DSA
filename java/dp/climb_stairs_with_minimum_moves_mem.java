...
public class climb_stairs_with_minimum_moves_mem {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println(getMinMoves(arr, 0, Integer.MAX_VALUE,new int[n]));
    }

    public static int getMinMoves(int[] arr, int idx, int msf,int[] strg) {
        if (idx == arr.length - 1) {
            return 0;
        }
        if (idx > arr.length - 1) {
        ...
        for (int i = 1; i <= arr[idx]; i++) {
            int moves = getMinMoves(arr, idx + i, msf,strg);
